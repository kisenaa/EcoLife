  ...
  return (
    <TouchableOpacity onPress={onNavigate} activeOpacity={0.85}>
      <ImageBackground
        ...
        style={themed($discoveryCard)}
        imageStyle={{ borderRadius: 16 }}
      >
        <View style={themed($discoveryGradientOverlay)}>
          <View style={themed($discoveryTextContainer)}>
            <Text preset="heading" style={themed($discoveryTitle)} numberOfLines={2} ellipsizeMode="tail">{item.title}</Text>
            <Text style={themed($discoveryDescription)} numberOfLines={3} ellipsizeMode="tail">{item.description}</Text>
          </View>
        </View>
      </ImageBackground>
    </TouchableOpacity>
